...
function selectFallbackProduct(need: any, products: RawProduct[]): Product | null {
  ...
  return {
    ...
    imageUrl: best.image || 'https://via.placeholder.com/300x300/f0f0f0/666666?text=No+Image',
    productUrl: best.url || '#',
...
function detectSingleItemQuery(query: string): boolean {
  const setupKeywords = ['setup', 'room', 'office', 'bedroom', 'kitchen', 'living room', 'gaming', 'workspace', 'studio', 'home', 'apartment']
  return !setupKeywords.some(keyword => query.toLowerCase().includes(keyword))
}
