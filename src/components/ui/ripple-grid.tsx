...
const RippleGrid: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    if (mouseInteraction) {
      // Listen on document to capture mouse movement even when pointer-events is none
  ...
  return (
    <div 
      ref={containerRef} 
      className={`absolute inset-0 w-full h-full overflow-hidden [&_canvas]:block ${className}`}
